...
if __name__ == '__main__':
    ...
    ra, dec = wcs.all_pix2world(hdu['RA'], hdu['DEC'], 0)    
    glade = LoadCataGalaxy(ra, dec, 3)
    glade = Limit_Coor_to_Fits(filefits, catalog = glade, ratext="RA", detext="dec")
    glade.write(filefits.replace('.fits', '-glad.csv'), format='csv')
...
#    print({"coor": [Glade['RA'], Glade['dec']]})
    ...
    p2, p98 = np.percentile(data, (20, 90))
    img_rescale = exposure.rescale_intensity(data, in_range=(p2, p98))
    x, y = wcs.all_world2pix(glade['RA'], glade['dec'], 0)
    
    plt.figure()
    plt.subplot(projection=wcs)
    plt.imshow(img_rescale, cmap='gray')
    plt.plot(x, y, 'b+')
    plt.savefig(filefits.replace('.fits', '-glade.png'), dpi=500)
    plt.show()
