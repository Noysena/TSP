...
def LoadCataGalaxy(RA, DEC, SR, Range):
...
#Convert  ASCII read to Table from CSV, not request by TSP
    ...
    keep_gal = []
    for i, j in enumerate(LCatalog['dist']):
        try:
            if float(j) < Range:
                keep_gal.append(i)        
        except(ValueError):
...
if __name__ == '__main__':
    import timeit
    import numpy as np
    from tarot.search.search_FoV import TAROT_FoV
    from tarot.utility.general_funtion import Plot_Image_Galaxy
    from tarot.utility.sub_fits import SUB_FITS

    data_path = '/home/kanthanakorn/Documents/Data/GrandMa_alert/'
    search_Gal = TAROT_FoV(data_path)
    search_Gal.List_File()
    search_Gal.FoV_Table()
    search_Gal.Predict_FoV()
    data = search_Gal.table
    start = timeit.default_timer()

    for i in data[7:8]:
        #export subfits
        fitsfile = os.path.join(data_path, i["FILENAME"])
        data, header = fits.getdata(fitsfile.replace('.fit', '.fits'), header=True)
        glade = LoadCataGalaxy(i['RA'], i['DEC'], 1.3, 130)
        print(glade)
        print(i['RA'], i['DEC'])
        
        #import cut fits and export fits
        fit= SUB_FITS(fitsfile.replace('.fit', '.fits'))
        
        print("-"*80)
        for j in glade:
            prefix = str(np.round(j['RA'].item(), 4)) + '_' + str(np.round(j['dec'].item(), 4))
            subfitname = '-' + prefix + '_gal.fits'
            savefit = fitsfile.replace('.fit', subfitname)
            print(savefit)
            
            #export echo subfits in coordinates
            fit.Sub_Fits(j['RA'], j['dec'], j['PGC'], savefit)
            
            #export subimage(png)
            ima_name =  fitsfile.replace('.fit', '') + '-'  + j['record_ID'] + '.png'
            fitnew = fitsfile.replace('.fit', '.fits')
            Plot_Image_Galaxy(fitnew, j['RA'], j['dec'], ima_name)
#                print(ima_name,fitnew)
